...
const providers = {
  local: {
    label: "Local",
    getSrc(id) {
      return `../mc_practice_audio/${id}.wav`;
    },
  },
  openrouter: {
    label: "OpenRouter",
    getSrc(id) {
      return `../teacher_tts_audio/openrouter/${id}.mp3`;
    },
  },
  grok: {
    label: "Grok",
    getSrc(id) {
      return `../teacher_tts_audio/grok/${id}.mp3`;
    },
  },
};
...
async function playLines(lines, label) {
  for (const line of lines) {
    ...
    await playAudio(line.id, `${label} line ${line.id}`);
  }
}
...
function renderLine(line) {
  ...
  if (line.hasAudio) {
    ...
    playButton.addEventListener("click", () => {
      playAudio(line.id, `Line ${line.id}`).catch(() => {});
    });

    const repeatButton = document.createElement("button");
    repeatButton.type = "button";
    repeatButton.className = "secondary";
    repeatButton.textContent = "Repeat x3";
    repeatButton.addEventListener("click", async () => {
      for (let count = 0; count < 3; count += 1) {
        await playAudio(line.id, `Line ${line.id} (${count + 1}/3)`).catch(() => {});
      }
    });

    actions.append(playButton, repeatButton);
  } else {
...
function renderPhase(phase) {
  ...
  if (phase.lines.length > 0) {
    ...
    playPhase.textContent = "Play Block";
...
async function loadData() {
  const response = await fetch("./data/mc_cue_index.json", { cache: "no-store" });
